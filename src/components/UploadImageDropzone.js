   ...
   const {uploadProgress, error, isSuccess} = useUploadImage(uploadSomeFile);
    ...
    return (
        <div {...getRootProps()} id="upload-image-dropzone-wrapper" 
            className={`text-center px-4 py-3 my-3 ${isDragAccept ? `drag-accept`: ``} ${isDragReject ? `drag-reject`: ``}`}>
            <input {...getInputProps()} /> 
            {
                isDragActive
                    ? isDragAccept ? <p>Please drop it! </p> : <p> Choose another file </p>
                    : <p> Drop your files </p>
            }
            {acceptedFiles && (
            ...
            {posting && (<Alert variant={posting.error ? 'warning' : 'success'}>{posting.text}</Alert>)}
